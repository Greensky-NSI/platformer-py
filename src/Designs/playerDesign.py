...
def draw_arm_profile(i=1, is_left=False):
    safe_fill(player_variables.player_colors["left_arm_color"] if is_left else player_variables.player_colors[
        "right_arm_color"])
    rect(10 * i, -14 * i, 12 * i, -20 * i)  # Bras en profil


def draw_leg_profile(i=1, is_left=False):
    safe_fill(player_variables.player_colors["right_leg_color"] if is_left else player_variables.player_colors[
        "left_leg_color"])
    rect(14 * i, 0, 8 * i, -12 * i)  # Jambe arrière en profil

    safe_fill(player_variables.player_colors["left_leg_color"] if is_left else player_variables.player_colors[
        "right_leg_color"])
